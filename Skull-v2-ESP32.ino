...
void makeHTTPCall(int buttonIndex)
{
  ...
  else
  {
    ...
    case -1:
      ...
      break;
    case -2:
      Serial.println("[HTTP] Send header failed");
      break;
    case -3:
      Serial.println("[HTTP] Send payload failed");
      break;
    case -11:
      Serial.println("[HTTP] Read timeout - serveur trop lent ou injoignable");
      break;
    default:
      Serial.println("[HTTP] Unknown error");
      break;
    }
  }
...
void handleRestart()
{
  ...
  server.send(200, "application/json", "{\"restarting\":true}"); // acknowledge before reboot
...
void setup()
{
  ...
  // API
  server.onNotFound([]
                    { addCORS(); server.send(404,"application/json","{\"error\":\"not found\"}"); });
  server.on("/", HTTP_GET, []
            { handleRoot(); });
  ...
  server.on("/api/relay", HTTP_POST, []
            { handleRelay(); });
  server.on("/api/auto-relay", HTTP_POST, []
            { handleAutoRelay(); });
  server.on("/api/button-config", HTTP_GET, []
            { handleButtonConfig(); });
  server.on("/api/button-config", HTTP_POST, []
            { handleButtonConfig(); });
  server.on("/api/restart", HTTP_POST, []
            { handleRestart(); });
  server.on("/api/relay", HTTP_OPTIONS, []
            { handleOptions(); });
  server.on("/api/auto-relay", HTTP_OPTIONS, []
            { handleOptions(); });
